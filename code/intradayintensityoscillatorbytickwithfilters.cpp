...
//SCDLLName("Intraday Intensity Oscillator By Tick With Filters") 
...
sc.StudyDescription = "Intraday Intensity Oscillator By Tick With Filters. To properly use this indicator, Sierra Chart must be set up in tick by tick mode with intraday bars. If all \"Input Data\" options show a straight line at 0.0, then tick by tick data is not available or not enabled. Note that some volume data is only available when the market is open.<p>A price and volume based indicator that attempts to estimate accumulation and distribution of institutional traders. Normal ranges are +/-0.3. Spikes can go higher. Filtering can changes these values some. This \"By Tick\" version uses the difference between up and down volume instead of the price scaling factor in the original. This version is more accurate.<p>This version includes extras to help clean up the graph and make it easier to integrate with automation. The \"Set IIO Smoothing?\" option enables cleaning up some of the IIO noise after the calculation. It is recommended to smooth IIO out with an EMA 3-7.<p>This version has a Volume Filter option to handle cases when a volume spike smashes the indicator making it hard to read. Large institutions sometimes do this to hide their activity afterward. The \"Straight\" option passes volume data through without any filtering. The \"Log\" option runs each volume bar through the log function making a type of \"log scale\". The \"Square Root\" option runs each volume bar through the square root function making a type of \"square root scale\". The \"MA Clip\" option will clip volume above a chosen moving average. To learn the movement characteristics of each, it is recommended to put them all on one chart for equal comparison.<p>To use traditional IIO, turn off all smoothing and filtering options.";
